...
const PRODUCTS = [
  {
    id: "hub-01",
    name: "Voyager hub",
    tag: "all-in-one charging",
    price: 29,
    icon: Cable,
    blurb: "One cable in, every port out. USB-C, USB-A, HDMI, all in a hub small enough to forget it's in your bag.",
    stamp: "BEST SELLER",
  },
  {
    id: "tracker-02",
    name: "Pathfinder tag",
    tag: "never lose it again",
    price: 24,
    icon: Radar,
    blurb: "Clip it to your bag, your passport pouch, your keys. Find it on a map from anywhere in the world.",
    stamp: "STAFF PICK",
  },
  {
    id: "battery-03",
    name: "Compass pack",
    tag: "card-sized power",
    price: 32,
    icon: BatteryCharging,
    blurb: "10,000mAh that slides into a back pocket. Two full phone charges before you even notice the weight.",
    stamp: null,
  },
  {
    id: "adapter-04",
    name: "Atlas adapter",
    tag: "every outlet, covered",
    price: 19,
    icon: Globe2,
    blurb: "150+ countries, one plug. Built for the person who packs light and moves often.",
    stamp: null,
  },
  {
    id: "glasses-05",
    name: "Horizon lenses",
    tag: "screen-fatigue relief",
    price: 28,
    icon: Glasses,
    blurb: "Blue-light filtering for the long work sessions in coffee shops and co-working spaces.",
    stamp: null,
  },
  {
    id: "pillow-06",
    ...
    price: 27,
    icon: Moon,
    blurb: "Memory foam that folds flat. Built for overnight buses, red-eyes, and everything in between.",
    stamp: null,
  },
];
...
function SuccessPage({ sessionId }) {
  return (
    <div style={{ background: "#F5F1E8", color: "#2B2B26", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <div className="max-w-md mx-auto px-6 pt-24 text-center">
        ...
        <p className="mt-3 text-sm leading-relaxed" style={{ color: "#5A564C" }}>
          Thanks — your payment went through and your order is on its way to dispatch.
          A confirmation has been sent to your email.
        </p>
        {sessionId && (
          <p className="mt-4 text-xs" style={{ color: "#8A8678" }}>
            Reference: {sessionId.slice(0, 20)}…
          </p>
        )}
        
          href="/"
          className="inline-block mt-8 px-6 py-3 rounded-full font-bold text-sm"
          style={{ background: "#2B2B26", color: "#F5F1E8" }}
        >
          Back to shop
        </a>
      </div>
    ...
    <div style={{ background: "#F5F1E8", color: "#2B2B26", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>
        ...
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div
              ...
              style={{ background: "#4A5D3A", color: "#F5F1E8" }}
            >
              N
            </div>
          ...
          >
            ...
            {itemCount > 0 && (
              <span
                ...
                style={{ background: "#C1551C", color: "#F5F1E8" }}
              >
                {itemCount}
              </span>
            )}
      ...
      <section className="max-w-5xl mx-auto px-6 pt-16 pb-12">
        <div className="flex items-start gap-4">
          <div
            className="hidden sm:flex shrink-0 w-20 h-20 rounded-full border-4 items-center justify-center font-black text-xs text-center rotate-[-8deg]"
            style={{ borderColor: "#C1551C", color: "#C1551C" }}
          >
            PACKED<br />LIGHT
          </div>
          <div>
            <h1 className="font-black tracking-tight leading-[0.95]" style={{ fontSize: "clamp(2.5rem, 6vw, 4.5rem)" }}>
              Gear that
              <br />
              <span style={{ color: "#4A5D3A" }}>travels</span> with you.
            </h1>
            <p className="mt-5 max-w-md text-base leading-relaxed" style={{ color: "#5A564C" }}>
              Six essentials for working and living out of a bag. No clutter, no dead weight — just the things that earn their place in your pack.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6 pb-20">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {visibleProducts.map((product) => {
            ...
            return (
              <div
                key={product.id}
                className="relative bg-white rounded-2xl p-6 border flex flex-col"
                style={{ borderColor: "#DBD4C2" }}
              >
                {product.stamp && (
                  <span
                    className="absolute top-4 right-4 text-[10px] font-black tracking-wider px-2 py-1 rounded-full rotate-[6deg]"
                ...
                <p className="text-xs font-bold tracking-wide uppercase mt-1" style={{ color: "#C1551C" }}>
                  {product.tag}
                </p>
                <p className="text-sm mt-3 leading-relaxed flex-1" style={{ color: "#5A564C" }}>
                  {product.blurb}
                </p>
        ...
        {!showAll && (
          <button
            ...
            className="mt-8 mx-auto flex items-center gap-2 text-sm font-bold underline underline-offset-4"
            style={{ color: "#4A5D3A" }}
          >
            See the full kit
            <ArrowRight size={14} />
      ...
      {cartOpen && (
        <div className="fixed inset-0 z-50 flex justify-end" style={{ background: "rgba(43,43,38,0.4)" }} onClick={() => setCartOpen(false)}>
          <div
            className="w-full max-w-sm h-full bg-white p-6 flex flex-col"
          ...
          >
            ...
            {cartItems.length === 0 ? (
              <p className="text-sm" style={{ color: "#5A564C" }}>
                Nothing in here yet. Add something from the kit.
              </p>
            ) : (
              <div className="flex-1 overflow-auto flex flex-col gap-4">
                {cartItems.map(({ product, qty }) => (
                  <div key={product.id} className="flex items-center gap-3 bg-white rounded-xl p-3 border" style={{ borderColor: "#DBD4C2" }}>
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => changeQty(product.id, -1)} aria-label="Decrease quantity" className="w-6 h-6 rounded-full border flex items-center justify-center" style={{ borderColor: "#DBD4C2" }}>
                        <Minus size={12} />
                      </button>
                      <span className="text-sm font-bold w-4 text-center">{qty}</span>
                      <button onClick={() => changeQty(product.id, 1)} aria-label="Increase quantity" className="w-6 h-6 rounded-full border flex items-center justify-center" style={{ borderColor: "#DBD4C2" }}>
                        <Plus size={12} />
                      </button>
                    </div>
                  </div>
            ...
            {cartItems.length > 0 && (
              <div className="pt-4 mt-4 border-t" style={{ borderColor: "#DBD4C2" }}>
                ...
                <input
                  ...
                  style={{ borderColor: "#DBD4C2", background: "white" }}
                />
                <button
                  onClick={handlePay}
                ...
                {status === "error" && (
                  <p className="text-xs mt-2 text-center" style={{ color: "#A32D2D" }}>
